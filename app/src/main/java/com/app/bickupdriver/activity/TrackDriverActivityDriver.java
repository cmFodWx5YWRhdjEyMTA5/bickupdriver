...
public class TrackDriverActivityDriver extends AppCompatActivity implements OnMapReadyCallback,
            ...
            call.enqueue(new Callback<RideObjectResponse>() {
                ...
                public void onResponse(Call<RideObjectResponse> call, Response<RideObjectResponse> response) {
                    ...
                    if (response.isSuccessful()) {
                        ...
                        ride = serverResponseRideDetails.ride;

                        /**
                         * Getting Status of Ride
                         */
                        getStatusOfRide();
    ...
    public static void zoomRoute(GoogleMap googleMap, List<LatLng> lstLatLngRoute) {
        ...
        try {
            ...
            googleMap.setPadding(100, 100, 100, 550);   // left, top, right, bottom
    ...
    private void setStatusOfRide(ArrayList<Status> statusList) {
        ...
        if (statusList.get(i).status == 7) {
            ...
            //this.onBackPressed();
            // Open Payment Screen
            openRideSummaryActivity();

        }
    }

    private void openRideSummaryActivity() {
        Intent rideSummaryIntent = new Intent(this, RideSummary.class);
        rideSummaryIntent.putExtra(ConstantValues.RIDE_ID, ride.rideId);
        startActivity(rideSummaryIntent);
        finish();
    }
